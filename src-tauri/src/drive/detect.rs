...
pub fn is_drive_desktop_path(
    ...
    match target_os {
        ...
        "windows" => {
            // Drive-letter form: `<L>:\My Drive\...`. The first byte must
            // be an ASCII letter — a bare `bytes[1] == b':'` check would
            // otherwise accept malformed inputs like `1:\My Drive\foo.md`.
            if s.len() >= 4 {
            ...
            // USERPROFILE form: `<home>\Google Drive\...`. Windows paths
            // are case-insensitive, so we lowercase both sides before
            // matching — mirroring the drive-letter branch above.
            if let Some(home) = home {
                let prefix_lower = format!(r"{}\Google Drive\", home).to_lowercase();
